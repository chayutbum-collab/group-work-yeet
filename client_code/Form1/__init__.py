...
class Form1(Form1Template):
  ...
  def _build_console(self):
    ...
    def _dashboard(self):
        ...
        live = Label(text='LIVE from Data Tables:  organizers=%d rows  |  events=%d rows  |  invoices=%d rows' % (len(app_tables.organizers.search()), len(app_tables.events.search()), len(app_tables.invoices.search())), foreground='#1a7f4b', bold=True, spacing_below='small')
        self.content.add_component(live)
        ...
        cards = [('Organizers', '{:,}'.format(len(self.orgs))), ('Active', '{:,}'.format(len([o for o in self.orgs if o['status'] == 'Active']))), ('Suspended', '{:,}'.format(len([o for o in self.orgs if o['status'] == 'Suspended']))), ('Events', '{:,}'.format(n_ev)), ('Participants', '{:,}'.format(n_part)), ('GMV (THB)', '{:,.0f}'.format(gmv)), ('Commission earned (THB)', '{:,.0f}'.format(earned)), ('Collected (THB)', '{:,.0f}'.format(collected)), ('Outstanding (THB)', '{:,.0f}'.format(outstanding)), ('Collection rate', '{:.0f}%'.format(coll_rate)), ('Take rate (comm/GMV)', '{:.1f}%'.format(take_rate)), ('Avg commission rate', '{:.1f}%'.format(avg_rate)), ('Invoices', '{:,}'.format(len(invs))), ('Due invoices', '{:,}'.format(len([i for i in invs if i['status'] == 'Due']))), ('Overdue invoices', '{:,}'.format(len([i for i in invs if i['status'] == 'Overdue']))), ('Avg GMV / event (THB)', '{:,.0f}'.format(avg_gmv)), ('Avg commission / event (THB)', '{:,.0f}'.format(avg_comm)), ('Avg participants / event', '{:,.0f}'.format(avg_part)), ('Avg ticket price (THB)', '{:,.0f}'.format(avg_price)), ('Events pending review', '{:,}'.format(len([e for e in evs if e['status'] == 'Pending'])))]
